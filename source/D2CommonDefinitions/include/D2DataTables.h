...
struct D2MonStats2Txt
{
	...
	//char szBaseW[4];					//0x10
	BYTE nHitClass;						//0x14
	BYTE unk0x15[16];					//0x15 - see CompositLinker
	BYTE unk0x25;						//0x25 - see CompositLinker
	D2UnkMonCompStrc unk0x26[16];		//0x26 - see CompositLinker
	BYTE pad0xE6[2];					//0xE6
	DWORD dwComponentFlags;				//0xE8
	BYTE nTotalPieces;					//0xEC
	BYTE pad0xED[3];					//0xED
	DWORD dwModeFlags;					//0xF0
	BYTE ndDT;							//0xF4
	BYTE ndNU;							//0xF5
	BYTE ndWL;							//0xF6
	BYTE ndGH;							//0xF7
...
struct D2NpcTxt
{
	...
	DWORD dwQuestFlag[3];				//0x10
	DWORD dwQuestSellMult[3];			//0x1C
	DWORD dwQuestBuyMult[3];			//0x28
	DWORD dwQuestRepMult[3];			//0x34
	int nMaxBuy[3];						//0x40
};
