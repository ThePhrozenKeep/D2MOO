...
int32_t __fastcall MISSMODE_SrvDo05_FireWall_ImmolationFire_MeteorFire_MoltenBoulderFirePath(D2GameStrc* pGame, D2UnitStrc* pMissile)
{
    ...
    const int32_t nCurrentFrame = pMissile->dwGFXcurrentFrame >> 8;
    ...
    {
        pMissile->dwGFXcurrentFrame = (nCurrentFrame + ITEMS_RollLimitedRandomNumber(&pMissile->pSeed, pMissilesTxtRecord->nSubStop - pMissilesTxtRecord->nSubStart)) << 8;
    }
    else if (MISSILE_GetCurrentFrame(pMissile) == pMissilesTxtRecord->nSubStart)
    {
        pMissile->dwGFXcurrentFrame = (pMissilesTxtRecord->nSubStart - 3) << 8;
    }
    else if (MISSILE_GetCurrentFrame(pMissile) < pMissilesTxtRecord->nSubStart)
    {
        const int32_t nCappedFrame = std::max(nCurrentFrame - 2, 0);
        pMissile->dwGFXcurrentFrame = nCappedFrame << 8;
    }
