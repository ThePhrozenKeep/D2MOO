...
int32_t __fastcall CLIENTS_AddPlayerToGame(D2ClientStrc* pClient, D2GameStrc* pGame, int32_t a3, int32_t a4, int32_t a5, int32_t a6)
{
    ...
    for (int32_t i = 0; i < 16; ++i)
    {
        ...
        {
            D2GAME_PACKETS_SendPacket0x7B_6FC3F720(pClient, i, pClient->HotkeySkills[i].nSkill, (uint8_t)pClient->HotkeySkills[i].__002, pClient->HotkeySkills[i].dwFlags);
        }
    }
...
D2ClientStrc* __fastcall CLIENTS_AddToGame(D2GameStrc* pGame, int32_t nClientId, uint8_t nCharTemplate, const char* szClientName, const char* szAccount, int32_t a6, uint32_t nExpLost, int32_t a8, int32_t a9)
{
    ...
    for (int32_t i = 0; i < 16; ++i)
    {
        ...
        pClient->HotkeySkills[i].__002 = 0;
...
void __fastcall CLIENTS_RemoveClientFromGame(D2GameStrc* pGame, int32_t nClientIdToRemove, BOOL bTriggerSave)
{
    ...
    if (gbClientListInitialized_6FD447E8)
    {
        ...
        strcpy(szName, pClientToRemove->szName);
...
void __fastcall CLIENTS_RefreshUnitsUpdateList(D2ClientStrc* pClient, uint32_t nUpdateSize)
{
    ...
    {
        for (D2UnitStrc* pRoomUnit = pAdjacentRoomsList[nCurrentRoom]->pUnitFirst; pRoomUnit != nullptr; pRoomUnit->pRoomNext)
        {
...
void __fastcall D2GAME_SetClientDead_6FC33830(D2ClientStrc* pClient, int32_t always0)
{
    ...
    if (always0)
    {
        pClient->unk0x190[2] = *(int32_t*)always0;
        pClient->unk0x190[3] = *(int32_t*)(always0 + 12);
        pClient->dwFlags |= 2;
    }
    else
    {
...
void __fastcall CLIENTS_SetSkillHotKey(D2ClientStrc* pClient, int32_t nHotkeyId, int16_t nSkillId, uint8_t a4, int32_t nFlags)
{
    if (pClient)
    {
        ...
        pClient->HotkeySkills[nHotkeyId].__002 = a4;
...
void __fastcall CLIENTS_GetSkillHotKey(D2ClientStrc* pClient, int32_t nId, int32_t* pSkillId, int32_t* a4, int32_t* pFlags)
{
    if (pClient)
    {
        ...
        *a4 = pClient->HotkeySkills[nId].__002;
