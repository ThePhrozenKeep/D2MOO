...
D2ClientStrc* __fastcall CLIENTS_AddToGame(D2GameStrc* pGame, int32_t nClientId, uint8_t nClassIdOrCharTemplate, const char* szClientName, const char* szAccount, int32_t nUnkCharacterId, uint32_t nLocale, int32_t a8, int32_t a9)
{
    ...
    pClient->nUnkCharacterId = nUnkCharacterId;
        ...
        if (gpD2EventCallbackTable_6FD45830)
        {
            if (D2_VERIFY(gpD2EventCallbackTable_6FD45830->pfLeaveGame))
            {
                gpD2EventCallbackTable_6FD45830->pfLeaveGame(
						...
						0,
						0,
						pClientToRemove->szAccount, pClientToRemove->nUnkCharacterId, &pClientToRemove->nLadderGUID);
            }
...
//D2Game.0x6FC346A0
void __fastcall D2GAME_GetUnkCharacterId_6FC346A0(D2ClientStrc* pClient, int32_t* pUnkCharacterId)
{
    *pUnkCharacterId = pClient->nUnkCharacterId;
}
