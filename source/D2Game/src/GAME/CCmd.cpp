...
void __fastcall CCMD_ProcessClientSystemMessage(void* pData, int32_t nSize)
{
    ...
    case D2CLTSYS_JOINGAME:
    {
        ...
        int32_t v18 = 0;
        int32_t v19 = 0;
        if (GAME_VerifyJoinGme(nClientId, pPacket67->nGameId, pPacket67->nPlayerClass, pPacket67->szClientName, pPacket67->nToken, szAccountName, &nUnkCharacterId, pPacket67->nLocale, &v19, &v18))
        {
            GAME_JoinGame(nClientId, pPacket67->nGameId, pPacket67->nPlayerClass, pPacket67->szClientName, szAccountName, nUnkCharacterId, pPacket67->nLocale, v19, v18);
        }
        else
        {
