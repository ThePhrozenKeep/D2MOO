	...
	if (pItem && pItem->dwUnitType == UNIT_ITEM && pItem->pItemData && pItem->pItemData->dwItemFlags & IFLAG_IDENTIFIED && !(pItem->pItemData->dwItemFlags & IFLAG_ETHEREAL))
	{
		...
		if (pItemsTxtRecord->wType[0] < sgptDataTables->nItemTypesTxtRecordCount)
		{
			...
			if (pItemTypesTxtRecord && pItemTypesTxtRecord->nRepair)
			{
				...
				if (nItemType < 0)
				{
					if (!pItemsTxtRecord->nNoDurability && pItemsTxtRecord->nDurability && STATLIST_GetMaxDurabilityFromUnit(pItem) && STATLIST_GetUnitStat(pItem, STAT_ITEM_INDESCTRUCTIBLE, 0) <= 0)
					{
						return TRUE;
					}
					return FALSE;
...
int __fastcall ITEMS_GetRequiredLevel(D2UnitStrc* pItem, D2UnitStrc* pPlayer)
{
	...
	nRequiredLevel += STATLIST_GetUnitStat(pItem, STAT_ITEM_LEVELREQ, 0);
	if (nRequiredLevel <= 0)
	{
		nRequiredLevel = 0;
	}
...
int __fastcall ITEMS_CalculateTransactionCost(D2UnitStrc* pPlayer, D2UnitStrc* pItem, int nDifficulty, D2BitBufferStrc* pQuestFlags, int nVendorId, int nTransactionType)
{
	...
	nLevel = STATLIST_GetUnitStat(pPlayer, STAT_LEVEL, 0);
	...
	pItemsTxtRecord = DATATBLS_GetItemsTxtRecord(pItem->dwClassId);
	nQuantity = STATLIST_GetUnitStat(pItem, STAT_QUANTITY, 0);
	if (nQuantity <= 0)
	{
		nQuantity = 1;
	}
	
	nReducePricePct = STATLIST_GetUnitStat(pPlayer, STAT_ITEM_REDUCEDPRICES, 0);
	...
	if (nTransactionType != 2)
	{
			...
			else
			{
				...
				else
				{
					...
					if (pItemTypesTxtRecord && pItemTypesTxtRecord->wQuiver)
					{
						...
						nStackQuantity = STATLIST_GetUnitStat(pItem, STAT_ITEM_EXTRA_STACK, 0) + pItemsTxtRecord->dwMaxStack;
					...
					else
					{
						...
						else
						{
							...
							nDivisor = STATLIST_GetUnitStat(pItem, STAT_ITEM_EXTRA_STACK, 0) + pItemsTxtRecord->dwMaxStack;
		...
		{
			if (pItemData->dwItemFlags & IFLAG_ETHEREAL && !pItemsTxtRecord->nNoDurability && pItemsTxtRecord->nDurability && STATLIST_GetMaxDurabilityFromUnit(pItem) && STATLIST_GetUnitStat(pItem, STAT_ITEM_INDESCTRUCTIBLE, 0) <= 0)
			{
				if (STATLIST_GetUnitStat(pItem, STAT_DURABILITY, 0) <= 0)
				{
					nBuyCost = 0;
				}
			}
		...
		else if (nTransactionType == 3)
		{
			...
			if (!pItemTypesTxtRecord || !pItemTypesTxtRecord->wQuiver || !pItemTypesTxtRecord->nThrowable)
			{
				if (!pItemsTxtRecord->nNoDurability && pItemsTxtRecord->nDurability && STATLIST_GetMaxDurabilityFromUnit(pItem) && STATLIST_GetUnitStat(pItem, STAT_ITEM_INDESCTRUCTIBLE, 0) <= 0)
				{
					nDurability = STATLIST_GetUnitStat(pItem, STAT_DURABILITY, 0);
					nMaxDura = STATLIST_GetMaxDurabilityFromUnit(pItem);

					if (nMaxDura && nDurability < nMaxDura)
					{
						if (STATLIST_GetUnitStat(pItem, STAT_ITEM_REPLENISH_DURABILITY, 0))
						{
		...
		if (!ITEMS_CheckItemTypeId(pItem, ITEMTYPE_BOOK))
		{
			...
			if (!pItemTypesTxtRecord || !pItemTypesTxtRecord->wQuiver)
			{
				...
				{
					nMaxStack = STATLIST_GetUnitStat(pItem, STAT_ITEM_EXTRA_STACK, 0) + pItemsTxtRecord->dwMaxStack;
					if (nMaxStack >= 511)
					{
						nMaxStack = 511;
					}

					if (nQuantity < nMaxStack && !STATLIST_GetUnitStat(pItem, STAT_ITEM_REPLENISH_QUANTITY, 0))
					{
...
int __stdcall ITEMS_GetTotalMaxStack(D2UnitStrc* pItem)
{
	...
	int nMaxStack = STATLIST_GetUnitStat(pItem, STAT_ITEM_EXTRA_STACK, 0) + pItemsTxtRecord->dwMaxStack;
	if (nMaxStack >= 511)
	{
		nMaxStack = 511;
	}
...
BOOL __stdcall ITEMS_HasDurability(D2UnitStrc* pItem)
{
	...
	if (pItem && pItem->dwUnitType == UNIT_ITEM)
	{
		...
		{
			return STATLIST_GetUnitStat(pItem, STAT_ITEM_INDESCTRUCTIBLE, 0) <= 0;
		}
	}
...
int __stdcall ITEMS_GetSockets(D2UnitStrc* pItem)
{
	...
	return STATLIST_GetUnitStat(pItem, STAT_ITEM_NUMSOCKETS, 0);
}
...
void __stdcall ITEMS_AddSockets(D2UnitStrc* pItem, int nSockets)
{
	...
	if (pItem && pItem->dwUnitType == UNIT_ITEM)
	{
		...
		if (nWidth * nHeight)
		{
			...
			case ITEMQUAL_SET:
			case ITEMQUAL_UNIQUE:
				nCurrentSockets = STATLIST_GetUnitStat(pItem, STAT_ITEM_NUMSOCKETS, 0);
...
BOOL __stdcall ITEMS_IsSocketable(D2UnitStrc* pItem)
{
	...
	return STATLIST_GetUnitStat(pItem, STAT_ITEM_NUMSOCKETS, 0) == 0;
}
...
int __stdcall ITEMS_GetAllRepairCosts(D2GameStrc* pGame, D2UnitStrc* pUnit, int nNpcId, int nDifficulty, D2BitBufferStrc* pQuestFlags, void(__fastcall* pfCallback)(D2GameStrc*, D2UnitStrc*, D2UnitStrc*))
{
	...
	while (nBodyLoc < 13)
	{
		...
		if (pItem)
		{
			...
			{
				pItemsTxtRecord = DATATBLS_GetItemsTxtRecord(pItem->dwClassId);
				if (pItemsTxtRecord && !pItemsTxtRecord->nNoDurability && pItemsTxtRecord->nDurability && STATLIST_GetMaxDurabilityFromUnit(pItem) && STATLIST_GetUnitStat(pItem, STAT_ITEM_INDESCTRUCTIBLE, 0) <= 0)
				{
					nMaxDurability = STATLIST_GetMaxDurabilityFromUnit(pItem);
					if (nMaxDurability && STATLIST_GetUnitStat(pItem, STAT_DURABILITY, 0) != nMaxDurability)
					{
						bCanBeRepaired = TRUE;
					}
				}
			...
			if (nItemType >= 0 && nItemType < sgptDataTables->nItemTypesTxtRecordCount)
			{
				...
				if (pItemTypesTxtRecord && pItemTypesTxtRecord->nRepair && pItem->dwUnitType == UNIT_ITEM && ITEMS_CheckItemTypeIfThrowable(ITEMS_GetItemType(pItem)))
				{
					if (ITEMS_CheckIfStackable(pItem) && !ITEMS_CheckItemFlag(pItem, IFLAG_ETHEREAL, __LINE__, __FILE__))
					{
						...
						if (STATLIST_GetUnitStat(pItem, STAT_QUANTITY, 0) < nMaxStack)
						{
							bCanBeRepaired = TRUE;
						}
					}
...
BOOL __stdcall ITEMS_AreStackablesEqual(D2UnitStrc* pItem1, D2UnitStrc* pItem2)
{
	...
	if((ITEMS_CheckItemFlag(pItem1, IFLAG_ETHEREAL, __LINE__, __FILE__) != ITEMS_CheckItemFlag(pItem2, IFLAG_ETHEREAL, __LINE__, __FILE__))
		...
		|| STATLIST_GetUnitStat(pItem1, STAT_MINDAMAGE, 0) != STATLIST_GetUnitStat(pItem2, STAT_MINDAMAGE, 0)
		|| STATLIST_GetUnitStat(pItem1, STAT_MAXDAMAGE, 0) != STATLIST_GetUnitStat(pItem2, STAT_MAXDAMAGE, 0)
		|| STATLIST_GetUnitStat(pItem1, STAT_SECONDARY_MINDAMAGE, 0) != STATLIST_GetUnitStat(pItem2, STAT_SECONDARY_MINDAMAGE, 0)
		|| STATLIST_GetUnitStat(pItem1, STAT_SECONDARY_MAXDAMAGE, 0) != STATLIST_GetUnitStat(pItem2, STAT_SECONDARY_MAXDAMAGE, 0)
		|| STATLIST_GetUnitStat(pItem1, STAT_ITEM_THROW_MINDAMAGE, 0) != STATLIST_GetUnitStat(pItem2, STAT_ITEM_THROW_MINDAMAGE, 0)
		|| STATLIST_GetUnitStat(pItem1, STAT_ITEM_THROW_MAXDAMAGE, 0) != STATLIST_GetUnitStat(pItem2, STAT_ITEM_THROW_MAXDAMAGE, 0))
	{
		return FALSE;
	}

	if (STATLIST_GetUnitStat(pItem1, STAT_ITEM_NUMSOCKETS, 0) || STATLIST_GetUnitStat(pItem2, STAT_ITEM_NUMSOCKETS, 0))
	{
		return FALSE;
	}
	else
...
BOOL __stdcall ITEMS_CanItemBeUsedForThrowSkill(D2UnitStrc* pItem)
{
	...
	if (pItem)
	{
		if (STATLIST_GetUnitStat(pItem, STAT_QUANTITY, 0) > 0)
		{
			return TRUE;
		}
		...
		nStack = STATLIST_GetUnitStat(pItem, STAT_ITEM_EXTRA_STACK, 0) + pItemsTxtRecord->dwMaxStack;
...
int __stdcall ITEMS_GetWeaponAttackSpeed(D2UnitStrc* pUnit, D2UnitStrc* pWeapon)
{
	...
	if (D2Common_10641(szPath, &nLength, &v11, &v13))
	{
		...
		return (nLength << 8) / (v11 * (STATLIST_GetUnitStat(pWeapon, STAT_ATTACKRATE, 0) + STATLIST_GetUnitStatSigned(pWeapon, STAT_ITEM_FASTERATTACKRATE, 0) + 100) / 100);
	}
...
void __fastcall ITEMS_SerializeItemCompact(D2UnitStrc* pItem, D2BitBufferStrc* pBuffer, D2ItemsTxt* pItemsTxtRecord, BOOL bServer)
{
	...
	else
	{
		...
		if (ITEMS_CheckItemTypeId(pItem, ITEMTYPE_GOLD))
		{
			nGold = STATLIST_GetUnitStat(pItem, STAT_GOLD, 0);
	...
	if (pItemsTxtRecord->nQuest && pItemsTxtRecord->nQuestDiffCheck)
	{
		...
		nValue = STATLIST_GetUnitStat(pItem, STAT_QUESTITEMDIFFICULTY, 0);
...
void __fastcall ITEMS_SerializeItemComplete(D2UnitStrc* pItem, D2BitBufferStrc* pBuffer, BOOL bServer, BOOL bGamble)
{
	...
	if (ITEMS_CheckItemTypeId(pItem, ITEMTYPE_ANY_ARMOR))
	{
		...
		{
			pItemStatCostTxtRecord = ITEMS_GetItemStatCostTxtRecord(STAT_DURABILITY);
			ITEMS_WriteBitsToBitstream(pBuffer, STATLIST_GetUnitStat(pItem, STAT_DURABILITY, 0) + pItemStatCostTxtRecord->dwSaveAdd, pItemStatCostTxtRecord->nSaveBits);
		}
	}
	else if (ITEMS_CheckItemTypeId(pItem, ITEMTYPE_WEAPON))
	{
	...
	{
		ITEMS_WriteBitsToBitstream(pBuffer, STATLIST_GetUnitStat(pItem, STAT_QUANTITY, 0), 9);
	}
