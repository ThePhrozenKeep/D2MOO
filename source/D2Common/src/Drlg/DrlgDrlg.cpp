...
void __fastcall DRLG_CreateRoomForRoomEx(D2DrlgStrc* pDrlg, D2RoomExStrc* pRoomEx)
{
	...
	DUNGEON_IsoTileToSubtileCoords(&pDrlgCoords.dwSubtilesLeft, &pDrlgCoords.dwSubtilesTop);
	DUNGEON_IsoTileToSubtileCoords(&pDrlgCoords.dwSubtilesWidth, &pDrlgCoords.dwSubtilesHeight);
	...
	for (D2RoomExStrc* pRoomEx = pLevel->pFirstRoomEx; pRoomEx; pRoomEx = pRoomEx->pRoomExNext)
	{
		...
		if (bHasWarp)
		{
			...
			*pY = pRoomEx->nTileYPos + pRoomEx->nTileHeight / 2;
			...
			DUNGEON_IsoTileToSubtileCoords(pX, pY);
